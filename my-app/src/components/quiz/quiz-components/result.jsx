...
const openNotification = elem => {
  setTimeout(function() {
    var data =
      ...
      encodeURIComponent(JSON.stringify(elem, null, 2));
...
class Result extends React.Component {
  ...
  renderQuestins() {
    return this.props.quizResult.map((_data, index) => {
      return (
        <div className="list-grp" key={index}>
          {_data.question} <br />
          Deine Auswahl: {this.props.answers[index].choice + 1}{" "}
    ...
    return (
      <div className="quiz-story">
        <div>
          ...
          {!this.props.store.online ? (
            <Button onClick={() => this.showdownload()}>
              Erstelle dein Ergebnis!
            </Button>
          ) : (
            undefined
          )}
          <a id="a" style={{ display: "none" }} />
          <a id="b" style={{ display: "none" }} />
          {this.props.store.online ? (
            <div id="container">
              <Button onClick={() => this.promptUpdate()}>Weiter</Button>
            </div>
          ) : (
  ...
  showdownload() {
    var data =
      "text/json;charset=utf-8," +
      encodeURIComponent(JSON.stringify(this.props.store.result, null, 2));
